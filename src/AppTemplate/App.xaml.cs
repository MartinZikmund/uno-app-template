    ...
    protected override async void OnLaunched(LaunchActivatedEventArgs args)
    {
        var builder = this.CreateBuilder(args)
            .Configure(host => host
                ...
                // EmbeddedSource<App>() loads the embedded appsettings.json (plus environment
                // overrides such as appsettings.development.json), and Section<AppConfig>() maps
                // its "AppConfig" section. The bound AppConfig is then injectable via
                // IOptions<AppConfig>. Release-time constants that do not vary per environment
                // live in ApplicationReleaseInfo instead.
